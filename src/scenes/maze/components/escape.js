...
export default class Escape {
  ...
  escape = () => {
    if (!this.parent.playTime.timer.paused || !this.parent.playTime.overrideAlpha) {
      this.parent.input.keyboard.once('keydown-SPACE', () => this.fadeSceneRestart(this.parent, true));
      this.parent.playTime.timer.paused = true;
      this.parent.player.paused = true;
      const startTime = this.parent.playTime.timerAlpha < 0 ? 0 : this.parent.playTime.timerAlpha;
      this.parent.playTime.overrideAlpha = this.parent.scene.systems.tweens.addCounter({from: startTime, to: 1, duration: 250});
